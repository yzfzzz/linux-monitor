...
void RpcClient::GetMonitorInfo(monitor::proto::MonitorInfo* monitor_info) {
    ...
    stub_ptr_->GetMonitorInfo(&controller, &request, monitor_info, nullptr);

    // 一次rpc调用完成, 读调用结果
    if (!controller.Failed()) {
        if (monitor_info->result().errcode() == 0) {
            // 调用正确
            std::cout << "rpc GetFriendsList response success!" << std::endl;
        } else {
            // 调用失败
            std::cout << "rpc GetFriendsList response error:" << monitor_info->result().errmsg()
                      << std::endl;
        }
    } else {
